    ...
    private function parse(array $row = array())
    {
        ...
        if(isset($row["header"]))
        {
            foreach(array_keys($row["header"]) as $key)
            {
                ...
                if($type == "reference" || $type == "reference_table")
                {
        ...
        if(isset($row["data"])){
            ...
            {
                $action = $this->action($data[$i]["id"]);
                $action_keys = array_keys($action);
                foreach($action_keys as $a)
                {
                    // $data[$i]["action"][$a]= $a;
                    $action[$a]["hint"] = __($name.".action.".$a.".hint");
                    $action[$a]["name"] = __($name.".action.".$a.".name");
                }
                ...
                foreach ($optimize as $key => $arr)
                {
                    foreach ($arr as $item)
                    {
                        if($key == "reference" || $key == "reference_table")
                        {
                        ...
                        {
                            if(isset($data[$i]["uuid"]) && isset($data[$i]["file_url"]) && isset($data[$i]["file_extension"]))
                            {
                                $data[$i]["upload_id"] = [[
                                    "id" => $data[$i]["upload_id"],
                                    "uuid" => $data[$i]["uuid"],
                                    "url" => $data[$i]["file_url"],
                                    "name" => isset($data[$i]["file_name"]) ? $data[$i]["file_name"] : "File",
                                    "extension" => $data[$i]["file_extension"],
                                ]];
                                unset($data[$i]["uuid"]);
                                unset($data[$i]["file_url"]);
                                unset($data[$i]["file_extension"]);
                            }
                        }
                    }
                }
